...
namespace DiagramDesigner
{
    ...
    internal class PathFinder
    {
        ...
        static float comparisonTolerance = 0.01f;

        internal static List<Point> GetConnectionLine(ConnectorInfo source, ConnectorInfo sink, bool showLastLine) //used for connecting two connectors that both of them are known
        {
            ...
            Point startPoint = CreatePointOutsideRect(source, rectSource); //it will be the second point after SetFirstAndLastPoints() function which is activated at the end of this function
            Point endPoint = CreatePointOutsideRect(sink, rectSink);
            ...
            SetFirstAndLastPoints(source, sink, showLastLine, linePoints, margin - 5);
            linePoints = OptimizeLinePoints(linePoints, new Rect[] { rectSource, rectSink }, source.Orientation, sink.Orientation); //makes connection orthogonal

            return linePoints;
        }

        internal static List<Point> GetConnectionLine(ConnectorInfo source, Point sinkPoint, ConnectorOrientation preferredOrientation) //used for connection adorner when one of connected elements is unknown
        {
            List<Point> linePoints = new List<Point>();
            Rect rectSource = GetRectWithMargin(source, margin);
            Point startPoint = CreatePointOutsideRect(source, rectSource);
            ...
            SetFirstPoint(source,linePoints,margin - 5);
        ...
        {
            if (Math.Abs(p1.X - p2.X) < comparisonTolerance)
            {
                if (p1.Y >= p2.Y)
            ...
            else if (Math.Abs(p1.Y - p2.Y) < comparisonTolerance)
            {
                if (p1.X >= p2.X)
        ...
        private static Point CreatePointOutsideRect(ConnectorInfo connector, Rect rect)
        {
        ...
        private static Point GetOffsetPoint(ConnectorInfo connector, double offset)
        {
            Point offsetPoint = new Point();
            ...
            {
                case ConnectorOrientation.Left:
                    offsetPoint = new Point(connector.Position.X - margin, connector.Position.Y);
                    break;
                case ConnectorOrientation.Top:
                    offsetPoint = new Point(connector.Position.X, connector.Position.Y - margin);
                    break;
                case ConnectorOrientation.Right:
                    offsetPoint = new Point(connector.Position.X + margin, connector.Position.Y);
                    break;
        ...
        private static void SetFirstAndLastPoints(ConnectorInfo source, ConnectorInfo sink, bool showLastLine, List<Point> linePoints, double marginPath)
        {
            if (showLastLine)
            {
        ...
        private static void SetFirstPoint(ConnectorInfo source, List<Point> linePoints, double marginPath)
        {
            Point startPoint = new Point(0, 0);
            switch (source.Orientation)
            {
